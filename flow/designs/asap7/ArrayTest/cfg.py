...
with open(os.path.join(DESIGN_DIR, 'config.mk'), 'w') as f:
    ...
    export('VERILOG_FILES_BLACKBOX', '{}/{}.v'.format(SRC_DIR, DESIGN_NAME))
...
PE_GAP = PE_DIE + 40
PE_GAP_TO_EDGE = PE_DIE / 2 + 20

# compute PE center positions and pin intervals
pe_pos_list = []
pe_interval_list = []
for r in range(N_PE_ROW):
    pos_list = []
    interval_list = []
    y = (N_PE_ROW - r - 1) * PE_GAP + PE_GAP_TO_EDGE
    y_interval = (y - (PIN_INTERVAL_SIZE) / 2, y + (PIN_INTERVAL_SIZE) / 2)

    for c in range(N_PE_ROW):
        x = PE_GAP_TO_EDGE + c * PE_GAP
        x_interval = (x - (PIN_INTERVAL_SIZE) / 2, x + (PIN_INTERVAL_SIZE) / 2)
        pos_list.append((x, y))
...
with open(os.path.join(DESIGN_DIR, 'io.tcl'), 'w') as f:
    ...
    for r in range(N_PE_ROW):
        for c in range(N_PE_ROW):
            x_interval, y_interval = pe_interval_list[r][c]
            evenly_place_pins(reverse_list(pin_left_list)[r], 'left', interval=y_interval, layers=(1,))
            evenly_place_pins(reverse_list(pin_right_list)[r], 'right', interval=y_interval, layers=(1,))
...
with open(os.path.join(DESIGN_DIR, 'mp.tcl'), 'w') as f:
    ...
    for r in range(N_PE_ROW):
        for c in range(N_PE_ROW):
            ...
            x, y = pe_pos_list[r][c]
            set_pos(name, x - PE_DIE / 2, y - PE_DIE / 2)
